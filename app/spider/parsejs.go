...
func (m *SpiderModel) Apply(sp *Spider) *Spider {
	if sp == nil {
		sp = &Spider{}
	}
	...
	sp.NotDefaultField = m.NotDefaultField
	sp.RuleTree = &RuleTree{Trunk: map[string]*Rule{}}
	...
	if m.Namespace != nil && !m.Namespace.IsEmpty() {
		sp.Namespace = func(self *Spider) string {
			vm := otto.New()
			m.Namespace.SetParams(vm, &FuncParam{
				Name: `self`,
				Data: self,
			})
	...
	if m.SubNamespace != nil && !m.SubNamespace.IsEmpty() {
		sp.SubNamespace = func(self *Spider, dataCell map[string]interface{}) string {
			vm := otto.New()
			m.SubNamespace.SetParams(vm, &FuncParam{
				Name: `self`,
				Data: self,
			}, &FuncParam{
				Name: `dataCell`,
...
func AddSpiderModels(spiderModels ...*SpiderModel) {
	for _, m := range spiderModels {
		m.Apply(nil).Register()
	}
}

func init() {
	AddSpiderModels(getSpiderModels()...)
}
