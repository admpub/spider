...
import (
	...
	"github.com/admpub/spider/common/goquery"
	"github.com/admpub/spider/common/mahonia"
	"github.com/admpub/spider/common/ping"
)
...
func ExtractArticle(html string) string {
	...
	html = strings.Replace(html, ss[idx][0], `<spider id="spider">`+ss[idx][1]+`</spider>`, -1)
	r := strings.NewReader(html)
	dom, err := goquery.NewDocumentFromReader(r)
	if err != nil {
		return ""
	}
	return dom.Find("spider#spider").Parent().Text()
}
	...
	if len(href) == 0 {
		...
		if count > 0 {
			...
			if len > 2 {
				...
				result = preUrl + strings.Replace(href, "../", "", -1)
			}
		} else {
			result = url + href
		}
